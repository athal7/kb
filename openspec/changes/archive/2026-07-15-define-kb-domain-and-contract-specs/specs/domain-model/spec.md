...
> Note: "project" and "product" are deliberately distinct concrete collections in the default schema pack, not synonyms. A product is an umbrella offering with real surface area and one or more repos; a project is a workstream with a parent product. The generic "Collection"/"Profile" vocabulary below describes the shared mechanism both are built from — it must not be read as blurring that distinction.
